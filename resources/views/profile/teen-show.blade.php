...
<head>
    ...
    <title>Pharenia - {{ __('Detalle del Joven (Teen)') }}</title>
...
<body>
    ...
    <div class="family-panel-container">
        ...
        <header class="family-panel__header">
            ...
            <h1>{{ __('Detalle del Joven (Teen)') }}</h1>
                ...
                <table class="pharenia-data-table">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td data-label="{{ __('Valor') }}" style="font-weight: bold; color: #718096;">{{ $teen->id }}</td>
                        </tr>
                        <tr>
                            <td data-label="{{ __('Campo') }}" style="font-weight: 600;">{{ __('Nombre') }}</td>
                            <td data-label="{{ __('Valor') }}" style="font-weight: 600;">{{ $teen->name }}</td>
                        </tr>
                        <tr>
                            <td data-label="{{ __('Campo') }}" style="font-weight: 600;">{{ __('Correo') }}</td>
                            <td data-label="{{ __('Valor') }}" style="color: #4a5568;">{{ $teen->email }}</td>
                        </tr>
                        <tr>
                            <td data-label="{{ __('Campo') }}" style="font-weight: 600;">{{ __('Fecha Nac.') }}</td>
                            <td data-label="{{ __('Valor') }}">{{ $teen->birthdate ? \Carbon\Carbon::parse($teen->birthdate)->toDateString() : 'N/D' }}</td>
                        </tr>
                        <tr>
                            <td data-label="{{ __('Campo') }}" style="font-weight: 600;">{{ __('Edad') }}</td>
                            <td data-label="{{ __('Valor') }}">{{ $teen->birthdate ? \Carbon\Carbon::parse($teen->birthdate)->age : 'N/D' }} {{ __('años') }}</td>
                        </tr>
                        <tr>
                            <td data-label="{{ __('Campo') }}" style="font-weight: 600;">{{ __('Rol') }}</td>
                            <td data-label="{{ __('Valor') }}"><span class="pharenia-role-badge">{{ $teen->role }}</span></td>
                        </tr>
                        <tr>
                            <td data-label="{{ __('Campo') }}" style="font-weight: 600;">{{ __('Supervisor ID') }}</td>
                            <td data-label="{{ __('Valor') }}" style="font-family: monospace;">{{ $teen->supervisor_id ?? '---' }}</td>
                        </tr>
                        <tr>
                            <td data-label="{{ __('Campo') }}" style="font-weight: 600;">{{ __('Supervisor') }}</td>
                            <td data-label="{{ __('Valor') }}">{{ $teen->supervisor ? $teen->supervisor->name : 'N/D' }}</td>
                        </tr>
                        <tr>
                            <td data-label="{{ __('Campo') }}" style="font-weight: 600;">{{ __('Registrado el') }}</td>
                            <td data-label="{{ __('Valor') }}">{{ $teen->created_at ? $teen->created_at->format('d/m/Y H:i') : 'N/D' }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 30px;">
            <a href="{{ route('admin.teens.edit', $teen->id) }}" class="pharenia-btn-primary" style="padding: 10px 18px; text-decoration: none;">{{ __('Editar') }}</a>
            <button type="button" class="pharenia-btn-danger" style="padding: 10px 18px;" onclick="openDeleteModal('{{ $teen->id }}', '{{ $teen->name }}')">{{ __('Eliminar') }}</button>
        </div>
    </div>
