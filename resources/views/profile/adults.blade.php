...
<head>
    ...
    <title>Pharenia - {{ __('Gestión de Adultos') }}</title>
...
<body>
    ...
    <div class="family-panel-container">
        ...
        <header class="family-panel__header">
            ...
            <h1>{{ __('Gestión de Adultos') }}</h1>
            <p>{{ __('Centro de control de los usuarios registrados como Adultos (TEA) y Tutores / Aliados (mayores de 18 años).') }}</p>
        </header>

        <!-- Métricas -->
        <div class="family-panel__grid pharenia-metrics">
            <div class="pharenia-card" style="text-align: center; padding: 15px;">
                <h3>{{ __('Total de Adultos') }}</h3>
                <p style="font-size: 24px; font-weight: bold; color: #2f4f4f; margin-top: 5px;">{{ $adults->count() }}</p>
            </div>
        </div>
        ...
        <div class="pharenia-card" style="margin-bottom: 25px;">
            <div class="pharenia-table-action-header">
                <h3 style="margin: 0; color: #2f4f4f;">{{ __('Listado de Adultos') }}</h3>
...
@forelse($adults as $adult)
                            <tr>
                                <td data-label="{{ __('ID') }}" style="font-weight: bold; color: #718096; text-align: center;">{{ $adult->id }}</td>
                                <td data-label="{{ __('Nombre') }}" style="font-weight: 600;">{{ $adult->name }}</td>
                                <td data-label="{{ __('Correo') }}" style="color: #4a5568;">{{ $adult->email }}</td>
                                <td data-label="{{ __('Fecha Nac.') }}">{{ $adult->birthdate ? \Carbon\Carbon::parse($adult->birthdate)->toDateString() : 'N/D' }}</td>
                                <td data-label="{{ __('Edad') }}">{{ $adult->birthdate ? \Carbon\Carbon::parse($adult->birthdate)->age : 'N/D' }} {{ __('años') }}</td>
                                <td data-label="{{ __('Rol') }}"><span class="pharenia-role-badge">{{ $adult->role }}</span></td>
                                <td data-label="{{ __('Acciones') }}" style="text-align: center;">
                                    <div style="display: flex; gap: 8px; justify-content: center;">
                                        <a href="{{ route('admin.adults.show', $adult->id) }}" class="pharenia-btn-secondary" style="padding: 6px 12px; font-size: 12px; text-decoration: none;">{{ __('Ver') }}</a>
                                        <a href="{{ route('admin.adults.edit', $adult->id) }}" class="pharenia-btn-primary" style="padding: 6px 12px; font-size: 12px; text-decoration: none;">{{ __('Editar') }}</a>
                                        <button type="button" class="pharenia-btn-danger" onclick="openDeleteModal('{{ $adult->id }}', '{{ $adult->name }}')">{{ __('Eliminar') }}</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" style="padding: 20px; text-align: center; color: #718096;">{{ __('No hay adultos registrados.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
