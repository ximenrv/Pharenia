...
<head>
    ...
    <title>{{ __('Estación de la Adultez') }} - Pharenia</title>
...
<body style="background-color: #f5efff; margin: 0; padding: 0;">
    ...
    @if(session()->has('active_child_id'))
        <div style="max-width: 600px; margin: 40px auto -70px auto; padding: 30px; border-radius: 12px; text-align: center; border: 1px solid #e9d5ff;">
            ...
            <p style="color: #4a5568; font-size: 15px; margin: 0 0 24px 0;">
                {{ __('Bienvenido a tu espacio seguro de adultez. ¡Disfruta tus actividades!') }}
            </p>
    ...
    <div class="stage-page">
        <div class="stage-container">
            <header class="stage-header" style="padding-top: 20px;">
                ...
                <h1 class="stage-header__title">{{ __('Estación de la Adultez') }}</h1>
            ...
            <div class="games-grid">
                <a href="#" class="game-card" data-game-url="{{ route('games.adults.ofertaoengano') }}" data-game-name="{{ __('Oferta o Engaño') }}">
                    <div class="game-card__image-wrapper">
                        <img src="{{ asset('gamesAssets/adults/ofertaoengano/ASSETS/backgrounds/ofertaoenganoport.png') }}" alt="{{ __('Oferta o Engaño') }}" class="game-card__img">
                        <div class="game-card__overlay" style="background-color: #7c4dff;">
                            <span class="game-card__play-btn">{{ __('¡JUGAR AHORA!') }}</span>
                        </div>
                    </div>
                    <div class="game-card__info">
                        <h3 class="game-card__title">{{ __('Oferta o Engaño') }}</h3>
                        <p class="game-card__description">{{ __('Aprende a identificar ofertas reales y engaños del día a día.') }}</p>
                    </div>
                </a>

                <a href="#" class="game-card" data-game-url="{{ route('games.adults.siguelareceta') }}" data-game-name="{{ __('Sigue la Receta') }}">
                    <div class="game-card__image-wrapper">
                        <img src="{{ asset('gamesAssets/adults/siguelareceta/assets/backgrounds/siguelarecetaport.png') }}" alt="{{ __('Sigue la Receta') }}" class="game-card__img">
                        <div class="game-card__overlay" style="background-color: #7c4dff;">
                            <span class="game-card__play-btn">{{ __('¡JUGAR AHORA!') }}</span>
                        </div>
                    </div>
                    <div class="game-card__info">
                        <h3 class="game-card__title">{{ __('Sigue la Receta') }}</h3>
                        <p class="game-card__description">{{ __('Aprende a cocinar paso a paso siguiendo las instrucciones.') }}</p>
                    </div>
                </a>
    ...
    <div class="modal-overlay" id="game-confirm-modal" aria-hidden="true">
        <div class="modal-box" role="dialog" aria-modal="true" aria-labelledby="modal-title">
            <h2 class="modal-title" id="modal-title">{{ __('¿Quieres jugar?') }}</h2>
            <div class="modal-actions">
                <button type="button" class="modal-btn modal-btn--cancel" id="modal-cancel">{{ __('Cancelar') }}</button>
                <button type="button" class="modal-btn modal-btn--confirm" id="modal-confirm">{{ __('¡Sí, jugar!') }}</button>
            </div>
        </div>
    </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.game-card').forEach(function (card) {
                card.addEventListener('click', function (e) {
                    ...
                    modalTitle.textContent = @json(__('¿Quieres jugar')) + ' ' + gameName + '?';
